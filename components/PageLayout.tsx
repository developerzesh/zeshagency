...
function ScrollProgress() {
  ...
  const smoothScale = useSpring(scaleX, { stiffness: 80, damping: 25, restDelta: 0.001 });

  useEffect(() => {
    const onScroll = () => {
      const scrollTop = document.documentElement.scrollTop || document.body.scrollTop;
      const scrollHeight = document.documentElement.scrollHeight - document.documentElement.clientHeight;
      scaleX.set(scrollHeight > 0 ? scrollTop / scrollHeight : 0);
    };
  ...
  return (
    <m.div style={{ scaleX: smoothScale }} className="fixed top-0 left-0 right-0 z-[100] h-[2px] bg-signal origin-left" />
  );
}
