...
export default function Philosophy() {
  ...
  return (
    <PageTransition>
      ...
      <div className="fixed top-0 left-0 right-0 z-50 h-[1px]"><m.div style={{ width: progressWidth }} className="h-full bg-signal" /></div>
          ...
          <div>
            {manifesto.map((item, i) => (
              <RevealText key={item.letter} delay={i * 0.08}>
                <m.div whileHover={{ x: 10 }} transition={{ duration: 1, ease: [0.22, 1, 0.36, 1] as [number, number, number, number] }} className="group flex items-start gap-8 md:gap-20 py-36 md:py-18 border-b border-border">
            ...
            <div className="md:col-span-5 md:col-start-8">
              {principles.map((p, i) => (
                <RevealText key={p.number} delay={i * 0.08}>
                  <m.div whileHover={{ x: 6 }} transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1] as [number, number, number, number] }} className="group py-10 border-b border-border">
      ...
      <section className="py-16 md:py-48 border-t border-border">
        <div className="max-w-[1400px] mx-auto px-4 md:px-16">
          ...
          <MagneticButton strength={0.15}><button onClick={() => setIsBookingOpen(true)} className="inline-flex items-center gap-3 bg-ink text-paper px-6 py-3 rounded-lg font-lato text-sm font-medium hover:bg-signal transition-colors duration-500">Start a Conversation →</button></MagneticButton>
        </div>
      </section>
