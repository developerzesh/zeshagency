...
export default function WorkDetail() {
  ...
  return (
    <PageTransition>
      ...
      <section className="py-16 md:py-48 border-t border-border"><div className="max-w-[1400px] mx-auto px-4 md:px-16"><RevealText><p className="font-lato text-[11px] tracking-[0.3em] uppercase text-signal mb-4">Impact</p></RevealText><RevealText delay={0.1}><h2 className="font-syne text-4xl md:text-6xl font-800 tracking-[-0.03em] mb-20 md:mb-32">Results<span className="text-signal">.</span></h2></RevealText><div className="grid grid-cols-2 md:grid-cols-4 gap-10 md:gap-14">{project.results.map((result, i) => (<RevealText key={result} delay={i * 0.1}><motion.div whileHover={{ y: -4 }} transition={{ duration: 1, ease: [0.22, 1, 0.36, 1] as [number, number, number, number] }}><span className="font-lato text-2xl md:text-4xl font-700 tracking-tight text-ink">{result.split(' ')[0]}</span><p className="font-lato text-sm text-text-secondary mt-2">{result.split(' ').slice(1).join(' ')}</p></motion.div></RevealText>))}</div></div></section>

      <section className="py-16 md:py-48 border-t border-border"><div className="max-w-[1400px] mx-auto px-4 md:px-16"><RevealText><h2 className="font-syne text-4xl md:text-6xl font-800 tracking-[-0.03em] mb-6">Like what you see<span className="text-signal">?</span></h2></RevealText><RevealText delay={0.1}><p className="font-lato text-base text-text-secondary max-w-md mb-8 leading-[1.85]">Every project starts with a conversation.</p></RevealText><MagneticButton strength={0.3}><a href="https://calendar.app.google/Mp8HrgYK67yjuYA29" target="_blank" rel="noopener noreferrer" className="font-lato text-sm font-medium text-signal sig-hover">Start a Project →</a></MagneticButton></div></section>
    </PageTransition>
  );
}
