    ...
    <PageTransition>
      <section ref={heroRef} className="relative min-h-[55vh] md:min-h-[70vh] flex items-end overflow-hidden pb-16 md:pb-28">
        ...
        <m.div style={{ y: heroY, opacity: heroOpacity }} className="relative z-10 max-w-[1400px] mx-auto px-4 md:px-16 pt-24 md:pt-40 w-full">
          <a href="/work" className="inline-flex items-center gap-2 font-lato text-[11px] tracking-[0.12em] uppercase text-text-muted hover:text-ink transition-colors duration-700 mb-10">â† Back to Work</a>
          <div className="flex flex-wrap items-center gap-4 mb-4"><span className="font-lato text-[11px] tracking-[0.12em] uppercase text-signal">{project.category}</span><span className="w-1 h-1 rounded-full bg-text-muted/20" /><span className="font-lato text-[11px] text-text-muted">{project.year}</span><span className="font-lato text-[11px] text-text-muted">Â· {project.client}</span></div>
